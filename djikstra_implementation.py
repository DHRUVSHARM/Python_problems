...
class Solution:
    def shortestPath(self, n: int, edges: List[List[int]], src: int) -> Dict[int, int]:
        ...
        distance, fixed = {node: float("inf") for node in range(n)}, {node: -1 for node in range(n)}
        distance[src] = 0
        ...
        for iteration in range(0, n):
            ...
            for node, dist in distance.items():
                if fixed[node] == -1 and (minimal_node is None or (dist < minimal_distance)):
            ...
            fixed[minimal_node] = -1 if minimal_distance == float("inf") else minimal_distance
