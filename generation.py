...
def find_generator_and_subgroups(modulus):
    ...
    subgroup_generators = {d: None for d in divisors(order)}  # Prepare a dictionary for subgroup generators
        ...
        for k in range(1, order + 1):
            if pow(a, k, modulus) == 1:
                if k in subgroup_generators and subgroup_generators[k] is None:
                    subgroup_generators[k] = a  # Assign a as a generator for the subgroup of size k
