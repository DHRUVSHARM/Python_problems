    ...
    def getLengthOfOptimalCompression(self, s: str, k: int) -> int:
        ...
        def dfs(index, k, prev_char, prev_count):
            ...
            if prev_char != s[index]:
                ...
                if k >= prev_count:
                    # can delete and make the entire thing zero
                    res = min(res, 0 + dfs(index + 1, k - prev_count, s[index], 1))
                if prev_count >= 1 and k >= prev_count - 1:
                    res = min(
                        res, 1 + dfs(index + 1, k - (prev_count - 1), s[index], 1)
                    )
                if prev_count >= 9 and k >= prev_count - 9:
                    res = min(
                        res, 2 + dfs(index + 1, k - (prev_count - 9), s[index], 1)
                    )
                if prev_count >= 99 and k >= prev_count - 99:
                    res = min(
                        res, 3 + dfs(index + 1, k - (prev_count - 99), s[index], 1)
                    )
                # cases for simple conversion
                if prev_count == 1:
                    res = min(res, 1 + dfs(index + 1, k, s[index], 1))
                if 2 <= prev_count <= 9:
                    res = min(res, 2 + dfs(index + 1, k, s[index], 1))
                elif 10 <= prev_count <= 99:
                    res = min(res, 3 + dfs(index + 1, k, s[index], 1))
                elif prev_count == 100:
                    res = min(res, 4 + dfs(index + 1, k, s[index], 1))
            else:
